...
export const authRuntimeStatus = {
  ...
  get coreConfigured() {
    return this.databaseConfigured && this.secretConfigured && this.baseURLConfigured;
  }
};
  ...
  plugins: [
    organization({
      ...
      async sendInvitationEmail(data) {
        const inviteLink = `${baseURL}/accept-invitation/${encodeURIComponent(data.id)}`;
        await sendAuthMail({
          to: data.email,
          subject: `Invitation til ${data.organization.name} i Skrivebord`,
          text: `${data.inviter.user.name} har inviteret dig til ${data.organization.name} i Skrivebord. Åbn invitationen: ${inviteLink}`
        });
      }
    }),
    apiKey([
    ...
    magicLink({
      ...
      async sendMagicLink({ email, url }) {
        await sendAuthMail({
          ...
          text: `Brug dette sikre link til at logge ind på Skrivebord: ${url}\n\nLinket udløber om 15 minutter.`
        });
      }
    })
  ]
